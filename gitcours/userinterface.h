...
void callhistorybytime(string timefilter1, string timefilter2) {
	system("cls");
	fstream H;
	ofstream callh;
	...
	if (H) {
		...
		while (a < 100) {
			...
			H >> calldatacheck4;
			H >> calldatacheck5;
			if (stoi(timefilter1) < stoi(calldatacheck3) && stoi(timefilter2) > stoi(calldatacheck3)) {
				valid = true;
		...
		if (!valid) {
			cout << "Не найдено ни одного звонка за указанный промежуток времени." << endl;
		}
		H.close();
		Rankusermenu();
	...
	if (oper == 4) {
		...
		if (UserMas[UserID].rate != "NONE") {
			...
			if (phonech == "1") {
				...
				cout << "1. За всё время" << endl;
				cout << "2. За определённый период" << endl;
				string callm;
				cin >> callm;
				if (callm == "1") {
					system("cls");
				...
				if (callm == "2") {
					system("cls");
					cout << "Для отображения информации о вызовах за определенный период необходимо указать следующие параметры:" << endl;
					cout << "Время, начиная с которого идет поиск звонков" << endl;
					cout << "Время, заканчивая которым останавливается поиск звокнов" << endl;
					string timefilter1, timefilter2;
					cout << "Введите первое значение в часах ( Пример: '12' )" << endl;
					cin >> timefilter1;
					cout << "Введите второе значение в часах ( Пример: '16' )" << endl;
					cin >> timefilter2;
					callhistorybytime(timefilter1, timefilter2);
